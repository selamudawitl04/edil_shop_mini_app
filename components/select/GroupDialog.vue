...
<template>
  <BaseDialog title="Select Group" v-model="open" body-class="max-w-xl">
    <template #content>
      <div class="h-80 space-y-3">
        <!-- 🔍 Search Input -->
        <BaseInput v-model="search" name="search" placeholder="Search groups by name" class="mb-4" />

        <!-- 👥 User List -->
        <ul v-if="groups.length" class="space-y-2">
          <li v-for="item in groups" :key="item.id" @click="selectGroup(item)" class="hover:cursor-pointer">
            <div
              ...
              <BaseAvatar :avatar_image="item.group.cover_image" :name="item.group.name"
                :avatar_color="item.group.avatar_color" class="w-10 h-10 rounded-full" />
              ...
              <div class="flex-1">
                ...
                <div class="text-xs text-textSecondary-light dark:text-textSecondary-dark">
                  {{ item.group.is_private ? "Private" : "Public" }}
                </div>
              </div>
            </div>
          </li>
