<template>
  <DevotionsCreate v-if="openCreateNewDevotionDialog" v-model="openCreateNewDevotionDialog" :community-id="group.id"
    community-type="group" :role="myRoleForGroup.role" :is-private-group="group.is_private" />
  ...
  <div class="space-y-3 my-4 lg:my-6 w-full">
    <div class="flex justify-between items-center  gap-2 px-0">
      ...
      <!-- Group admin or owner can create devotion -->
    ...
    <div v-if="activeIndex != 1 && devotions.length" class="grid gap-6 md:grid-cols-1">
      <DevotionsCard v-for="devotion in devotions" :key="devotion.id" :devotion="devotion" :role="myRoleForGroup?.role"
        :show-series="true" />
    </div>
...
const canCreateDevotion = computed(() => {
  return props.myRoleForGroup.role === "group:admin" || props.myRoleForGroup.role === "group:owner" || (props.myRoleForGroup.role === "group:member" && props.group?.allow_members_to_create_devotion);
});
